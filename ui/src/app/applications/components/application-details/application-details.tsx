...
import {AppContext, ClusterSettingsCtx, ContextApis} from '../../../shared/context';
...
import {ChartDetails, ClusterSettings, ResourceStatus} from '../../../shared/models';
...
export class ApplicationDetails extends React.Component<RouteComponentProps<{appnamespace: string; name: string}>, ApplicationDetailsState> {
                                        ...
                                        {clusterSettings => (
                                            <Page
                                                ...
                                                toolbar={{
                                                    ...
                                                    actionMenu: {items: this.getApplicationActionMenu(application, true, clusterSettings)},
                                                    tools: (
                                                                ...
                                                                <ApplicationResourceTree
                                                                    ...
                                                                    nodeMenu={node =>
                                                                        AppUtils.renderResourceMenu(node, application, tree, this.appContext.apis, this.appChanged, () =>
                                                                            this.getApplicationActionMenu(application, false, clusterSettings)
                                                                        )
                                                                    }
    ...
    private getApplicationActionMenu(app: appModels.Application, needOverlapLabelOnNarrowScreen: boolean, clusterSettings: ClusterSettings) {
        ...
        const parentApp: string = app?.metadata?.labels?.[clusterSettings?.appLabelKey]
        if (parentApp) {
            menuItems.push({
                iconClassName: 'fa fa-level-up',
                title: <ActionMenuItem actionLabel="Parent" />,
                action:async () => {
                    this.appContext.apis.navigation.goto(`/applications/${parentApp}`)
                },
                disabled: !app.status.operationState
            })
        }
